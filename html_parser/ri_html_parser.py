...
class RIParseHtml(ParserBase):
    def __init__(self,input_file_name):
        ...
        self.dictionary_to_store_class_name = {'h1': r'^Title \d+[A-Z]?(\.\d+)?', 'h4': 'Compiler’s Notes\.',
                                               ...
                                               'li': r'^Chapters? \d+(\.\d+)?(\.\d+)?',
                                               'h3': r'^\d+[A-Z]?(\.\d+)?-\d+(-\d+)?(\.\d+(-\d+)?(\.\d+)?(-\d+)?)?|Chs. \d+  - \d+\.',
                                               'h2': r'^Chapters? \d+(\.\d+)?(\.\d+)?',
    ...
    def convert_to_header_and_assign_id(self):
        ...
        for tag in self.soup.find_all("p"):
            ...
            if class_name == self.dictionary_to_store_class_name['h1']:
                ...
                if re.search("^Chapters? \d+(\.\d+)?(\.\d+)?", tag.text.strip()):
                    tag.name = "h2"
                    chapter_number = re.search("^Chapters? (?P<chapter_number>\d+(\.\d+)?(\.\d+)?)",
                ...
                tag.name = "h3"
                tag['class'] = "section"
                if re.search("^\d+[A-Z]?(\.\d+)?-\d+(-\d+)?(\.\d+(-\d+)?(\.\d+)?(-\d+)?)?|^Chs. \d+  - \d+\.",
                             tag.text.strip()):
                    if re.search("^\d+[A-Z]?(\.\d+)?-\d+(-\d+)?(\.\d+(-\d+)?(\.\d+)?(-\d+)?)?", tag.text.strip()):
                        id_of_section = re.search("^\d+[A-Z]?(\.\d+)?-\d+(-\d+)?(\.\d+(-\d+)?(\.\d+)?(-\d+)?)?",
                ...
                if tag.text.strip() in list_to_store_regex_for_h4:
                    ...
                    if tag.find_previous_sibling().attrs['class'][0] == self.dictionary_to_store_class_name[
                        ...
                        tag.attrs[
                            'id'] = f"{tag.find_previous_sibling('h3').attrs['id']}-{re.sub(r'[^a-zA-Z0-9]', '', tag.text).lower()}"
                if tag.text.strip() == 'NOTES TO DECISIONS':
                    ...
                    for sub_tag in tag.find_next_siblings():
                        ...
                        elif re.search('^\d+[A-Z]?(\.\d+)?-\d+(-\d+)?(\.\d+(-\d+)?(\.\d+)?(-\d+)?)?|^Chs. \d+  - \d+\.',
                ...
                if re.search("^Chapters? \d+(\.\d+)?(\.\d+)?", tag.text.strip()) or re.search(
                        "^\d+[A-Z]?(\.\d+)?-\d+(-\d+)?(\.\d+(-\d+)?(\.\d+)?(-\d+)?)?|^Chs. \d+  - \d+\.",
        ...
        for li_tag in self.soup.find_all("li"):
            if re.search("^Chapters? \d+(\.\d+)?(\.\d+)?", li_tag.text.strip()):
                ...
                li_tag.wrap(ul_tag_for_chapter)
                    ...
                    "^\d+[A-Z]?(\.\d+)?-\d+(-\d+)?(\.\d+(-\d+)?(\.\d+)?(-\d+)?)?|^Chs. \d+  - \d+\.|^Article \d+",
                    li_tag.text.strip()):
                nav_tag_for_section_ul = self.soup.new_tag("nav")
                if re.search("^\d+[A-Z]?(\.\d+)?-\d+(-\d+)?(\.\d+(-\d+)?(\.\d+)?(-\d+)?)?", li_tag.text.strip()):
                    section_id = re.search("^\d+[A-Z]?(\.\d+)?-\d+(-\d+)?(\.\d+(-\d+)?(\.\d+)?(-\d+)?)?",
    ...
    def add_citation(self):
        for tag in self.soup.find_all(["p", "li"]):
            if tag['class'] not in ["notes_to_decision", "notes_section"]:
                ...
                if re.search(
                        '\d+[A-Z]?(\.\d+)?-\d+(-\d+)?(\.\d+(-\d+)?(\.\d+)?(-\d+)?)?((\([a-z 0-9 (ix|iv|v?i{0,3}) A-Z 0-9]{1,3}\) ?)+)?',
                        tag.text.strip()) and tag.attrs['class'] != "nav_li":
                    for pattern in sorted(set(match[0] for match in re.findall(
                            '(\d+[A-Z]?(\.\d+)?-\d+(-\d+)?(\.\d+(-\d+)?(\.\d+)?(-\d+)?)?((\([a-z 0-9 (ix|iv|v?i{0,3}) A-Z 0-9]{1,3}\) ?)+)?)',
                            tag.text.strip()))):
                        section_match = re.search(
                            "(?P<section_id>(?P<title_id>\d+[A-Z]?(\.\d+)?)-(?P<chapter_id>\d+)((-\d+)?(\.\d+(-\d+)?(\.\d+)?(-\d+)?)?))",
                            pattern)
                        file_exists = exists(
                        ...
                        if file_exists:
                            ...
                            continue
                        if re.search(
                                f'\d+[A-Z]?(\.\d+)?-\d+(-\d+)?(\.\d+(-\d+)?(\.\d+)?(-\d+)?)?(\([a-z 0-9 (ix|iv|v?i{0, 3}) A-Z ]+\) ?)+',
                                pattern.strip()):
                            ...
                            if re.search('(?<!\.|-)\d+[A-Z]?(\.\d+)?-\d+(?!((\d)?\.\d+)|((\d)?-\d+))',
                                ...
                                if re.search(f'id=".+c{chapter_id}"', content):
                                    ...
                                    tag_string = re.sub(fr'{re.escape(pattern)}',
                                                        f'<cite class="ocri"><a href=http://localhost:63342/practice/venv/ricode/practice/gov.ri.code.title.{section_match.group("title_id").zfill(2)}.html?_ijt=lartillgujbilc2c7ak6tlmhr8&_ij_reload=RELOAD_ON_SAVE#{tag_id} target="_self">{pattern}</a></cite>',
                                                        text)
                            else:
                                if re.search('\d+[A-Z]?(\.\d+)?-\d+-\d(?!(\d+))', pattern):
                                    if re.search(f'id=".+s{pattern}"', content):
                                        tag_id = re.search(f'id="(?P<tag_id>(.+s{pattern}))"', content).group('tag_id')
                                        tag_string = re.sub(fr'{re.escape(pattern)}' + '(?!((\d)?\.\d+)|(\d+))',
                                                            f'<cite class="ocri"><a href=http://localhost:63342/practice/venv/ricode/modified/gov.ri.code.title.{section_match.group("title_id").zfill(2)}.html?_ijt=lartillgujbilc2c7ak6tlmhr8&_ij_reload=RELOAD_ON_SAVE#{tag_id} target="_self">{pattern}</a></cite>',
                                                            text)
                                elif re.search('\d+[A-Z]?(\.\d+)?-\d+-\d+(?!\.\d+)', pattern):
                                    if re.search(f'id=".+s{pattern}"', content):
                                        tag_id = re.search(f'id="(?P<tag_id>(.+s{pattern}))"', content).group('tag_id')
                                        tag_string = re.sub(fr'{re.escape(pattern)}' + '(?!((\d)?\.\d+))',
                                                            f'<cite class="ocri"><a href=http://localhost:63342/practice/venv/ricode/modified/gov.ri.code.title.{section_match.group("title_id").zfill(2)}.html?_ijt=lartillgujbilc2c7ak6tlmhr8&_ij_reload=RELOAD_ON_SAVE#{tag_id} target="_self">{pattern}</a></cite>',
                                                            text)
    ...
    def create_ol_tag(self):
        ...
        for tag in self.soup.main.find_all("p"):
            ...
            if class_name == self.dictionary_to_store_class_name['History'] or class_name == \
                    self.dictionary_to_store_class_name['ol_of_i'] or self.dictionary_to_store_class_name['h4']:
                ...
                if next_tag.name != 'h4':
                    if re.search(f'^{number}\.', tag.text.strip()):
                        ...
                        if re.search('OFFICIAL COMMENT|COMMISSIONER’S COMMENT',
                            ...
                            tag['id'] = f"{tag.find_previous_sibling('h3').attrs['id']}ol{ol_count}{number}"
                        tag.wrap(ol_tag_for_number)
                        number += 1
                        while (next_tag.name != "h4" and next_tag.name != "h3") and re.search('^“?[a-z A-Z]+',
                                                                                              next_tag.text.strip()):
                        ...
                        if re.search(f'^\({caps_alpha}\) \({caps_roman}\) \({inner_alphabet}\)', tag.text.strip()):
                            tag.name = "li"
                            text = str(tag)
                            tag_string = re.sub(
                                f'^<li[^>]*>(<span.*</span>)?<b>\({caps_alpha}\) \({caps_roman}\) \({inner_alphabet}\)</b>|^<li[^>]*>(<span.*</span>)?\({caps_alpha}\) \({caps_roman}\) \({inner_alphabet}\)|</li>$',
                                '', text.strip())
                            tag.clear()
                            tag.append(BeautifulSoup(tag_string, features="html.parser"))
                            ...
                            li_tag_for_caps_alpha[
                                'id'] = f"{ol_tag_for_number.find_all('li', class_='number')[-1].attrs['id']}-{caps_alpha}"
                            li_tag_for_caps_roman = self.soup.new_tag("li")
                            li_tag_for_caps_roman[
                                'id'] = f"{ol_tag_for_number.find_all('li', class_='number')[-1].attrs['id']}-{caps_alpha}{caps_roman}"
                            li_tag_for_caps_alpha['class'] = "caps_alpha"
                            tag.attrs[
                                'id'] = f"{ol_tag_for_number.find_all('li', class_='number')[-1].attrs['id']}-{caps_alpha}{caps_roman}{inner_alphabet}"
                            ...
                            tag['id'] = f"{id_of_last_li}-{caps_alpha}"
                            if caps_alpha == "Z":
                                caps_alpha = 'A'
                            else:
                                caps_alpha = chr(ord(caps_alpha) + 1)
                            if re.search('^[a-z A-Z]+', next_tag.text.strip()):
                                while re.search("^[a-z A-Z]+", next_tag.text.strip()):
                                    ...
                                    next_tag = sub_tag
                                count_of_p_tag = 1
                            if re.search(f'^\({inner_roman}\)', next_tag.text.strip()):
                                continue
                                ...
                                if ol_tag_for_roman.li:  # 1aiAb
                                    ol_tag_for_roman.find_all("li", class_="roman")[-1].append(ol_tag_for_caps_alphabet)
                                    ol_tag_for_inner_alphabet.find_all("li")[-1].append(ol_tag_for_roman)
                                    ol_tag_for_caps_alphabet = self.soup.new_tag("ol", type="A")
                                    caps_alpha = 'A'
                                    ol_tag_for_roman = self.soup.new_tag("ol", type="i")
                                    roman_number = 'i'
                        ...
                        if ol_tag_for_inner_number.li:
                            id_of_last_li = ol_tag_for_inner_number.find_all("li", class_="inner_num")[-1].attrs[
                                'id']
                        ...
                        if re.search(f'^\({number}\)', next_tag.text.strip()):
                            ...
                            continue
                    elif re.search(f'^\({roman_number}\)', tag.text.strip()) and (
                            ol_tag_for_number.li or alphabet != roman_number):
                        if re.search(f'^\({roman_number}\) \({caps_alpha}\)', tag.text.strip()):
                            tag.name = "li"
                            text = str(tag)
                            tag_string = re.sub(
                            ...
                            tag.attrs['id'] = f"{id_of_last_li}{roman_number}-{caps_alpha}"
                            caps_alpha = chr(ord(caps_alpha) + 1)
                            ...
                            while next_tag.name != "h4" and (re.search('^“?[a-z A-Z]+',
                                                                       next_tag.text.strip()) or next_tag.next_element.name == "br"):  # 5-31.1-1. after 16i 2 break
                        ...
                        if re.search(f'^\({alphabet}\) \({number}\)', tag.text.strip()):
                            ...
                            text = str(tag)
                            tag_string = re.sub(
                                '^<li[^>]*>(<span.*</span>)?<b>\([a-z]\)( \(\d\))?</b> (\(\d+\))?|^<li[^>]*>(<span.*</span>)?\([a-z]\) \(\d\)|</li>$',
                                '', text.strip())
                            tag.clear()
                            tag.append(BeautifulSoup(tag_string, features="html.parser"))
                            tag.wrap(ol_tag_for_number)
                            ...
                            if ol_tag_for_number.li:
                                ...
                                if comment_count == 0:
                                    ...
                                    tag.attrs[
                                        'id'] = f"{tag.find_previous_sibling('h4').attrs['id']}ol{ol_count}{alpha_id}"
                                if alphabet == "z":
                                    alphabet = 'a'
                                else:
                                    alphabet = chr(ord(alphabet) + 1)
                                ...
                                while (next_tag.name != "h4" and next_tag.name != "h3" and not re.search(
                                        ...
                                        re.search('^“?[a-z A-Z]+|^\(Name .*\)',
                                                  next_tag.text.strip()) or next_tag.next_element.name == "br"):
                                    if next_tag.next_element.name == "br":
                                        sub_tag = next_tag.find_next_sibling()
                                        next_tag.decompose()
                                        next_tag = sub_tag
                        ...
                        if re.search(f'^\({number}\) \({roman_number}\) \({caps_alpha}\)', tag.text.strip()):
                            tag.name = "li"
                            text = str(tag)
                            tag_string = re.sub(
                            ...
                            li_tag_for_roman['id'] = f"{h3_id}ol{ol_count}{roman_number}"
                            li_tag_for_number['class'] = "number"
                            tag.attrs['id'] = f"{h3_id}ol{ol_count}{number}{roman_number}-{caps_alpha}"
                            ...
                            number += 1
                        elif re.search(f'^\({number}\) \({caps_alpha}\)', tag.text.strip()):
                            tag.name = "li"
                            text = str(tag)
                            tag_string = re.sub(
                            ...
                            li_tag['class'] = "number"
                            li_tag.append(ol_tag_for_caps_alphabet)
                            tag.attrs['id'] = f"{h3_id}ol{ol_count}{number}-{caps_alpha}"
                            caps_alpha = chr(ord(caps_alpha) + 1)
                            ol_tag_for_number.append(li_tag)
                            number += 1
                            ...
                            if re.search('^Section \d+', next_tag.text.strip()):
                                ...
                                number = 1
                                ol_count = 1
                            elif re.search("^\([A-Z a-z]+\)\.”|^\. \. \.|^“?[a-z A-Z]+",
                                           next_tag.text.strip()) or next_tag.next_element.name == "br":
                    ...
                    if re.search(f'^{number}\.', tag.text.strip()):
                        ...
                        caps_id = re.search(f'^\((?P<caps_id>{caps_alpha}{caps_alpha}?)\)', tag.text.strip()).group(
                            'caps_id')
                        tag.name = "li"
                        text = str(tag)
                        tag_string = re.sub(
                        ...
                        if ol_tag_for_roman.li:
                            id_of_last_li = ol_tag_for_roman.find_all("li", class_="roman")[-1].attrs['id']
                            tag['id'] = f"{id_of_last_li}-{caps_id}"
                            ol_tag_for_roman.find_all("li", class_="roman")[-1].append(ol_tag_for_caps_alphabet)
                            ol_tag_for_number.find_all("li", class_="number")[-1].append(ol_tag_for_roman)
                            if ol_tag_for_alphabet.li:
                                ol_tag_for_alphabet.find_all("li", class_="alphabet")[-1].append(ol_tag_for_number)
                ...
                if next_tag.name == "nav":
                    ...
                    if next_tag.name == "h3":
                        ...
                        while tag_of_h3 and (tag_of_h3.name != "h2" or (
                                tag_of_h3.name == "h2" and not re.search('^Chapters? \d+(\.\d+)?(\.\d+)?',
                            ...
                            if tag_of_h3.name == "h4":
                                ...
                                while tag_of_h4 and tag_of_h4.name != "h4" and (tag_of_h4.name != "h2" or (
                                        tag_of_h4.name == "h2" and not re.search('^Chapters? \d+(\.\d+)?(\.\d+)?',
                                    ...
                                    if tag_of_h4.name == "h3":
                                        ...
                                        while tag_of_h5.name != "h5" and tag_of_h5.name != "h3":
                                            if tag_of_h5.next_element.name == "br":
                                                ...
                                                div_tag_for_h4 = self.soup.new_tag("div")
                                                break
                                            elif tag_of_h5.name == "h2" and re.search('^Chapters? \d+(\.\d+)?(\.\d+)?',
                                        ...
                                        div_tag_for_section.append(tag_of_h4)
                                        tag_of_h4 = next_tag
                                        if tag_of_h4.name == "h2" and re.search('^Chapters? \d+(\.\d+)?(\.\d+)?',
                                                                                tag_of_h4.text.strip()):
                                            if div_tag_for_part.next_element:
                                                div_tag_for_part.append(div_tag_for_section)
                                                div_tag_for_chapter.append(div_tag_for_part)
    ...
    def add_watermark(self):
        ...
        meta_tag_for_water_mark.attrs[
            'content'] = "Release 70 of the Official Code of Rhode Island Annotated released 2021.11.Transformed and posted by Public.Resource.Org using rtf-parser.py version 1.0 on 2022-06-13.This document is not subject to copyright and is in the public domain."
    ...
    def write_to_file(self):
        ...
        with open(f"../../cic-code-ri/transforms/ri/ocri/r{self.release_number}/{self.html_file_name}", "w") as file:
